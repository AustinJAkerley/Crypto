...
from crypto.signatures.number_field.rsa_sig import rsa_sig
...
class TestRSASignature(unittest.TestCase):
    def test_rsa_signature_1(self):
        ...
        new_signature.gen_e()
        e = new_signature.e
        signature = new_signature.sign("crypto/signatures/number_field/unit_test/test_files/example_doc.txt")

        signature_verifier = rsa_sig()
        is_signature_valid = signature_verifier.verify(signature, "crypto/signatures/number_field/unit_test/test_files/example_doc.txt", e, N)
    ...
    def test_rsa_signature_2(self):
        ...
        signature_verifier = rsa_sig()
        is_signature_valid = signature_verifier.verify(signature, "crypto/signatures/number_field/unit_test/test_files/example_doc_bad.txt", e, N)
